...
makeLesionCountTable <- function(df, names_use, time="MRI", use_type="ICH", count_type="percent") {
  ...
  for (i in 1:length(names_use)) {
    ...
    if (time == "MRI") {
      ...
      #edemaval_cIps <- row_use$cDC_pcnt[row_use$type == "edema" & grepl("ipsi", row_use$variable)]
      ...
      if (names_use[i] %in% c("MB", "AntPons", "Teg")) {
        val_cIps <- (row_use$cDC_pcnt[row_use$type == use_type & grepl("_C", row_use$variable)])
        #edemaval_cIps <- row_use$cDC_pcnt[row_use$type == "edema" & grepl("ipsi", row_use$variable)]
        val_cCont <- (row_use$cDC_pcnt[row_use$type == use_type & grepl("_C", row_use$variable)])
        val_ucIps <- (row_use$ucDC_pcnt[row_use$type == use_type &  grepl("_C", row_use$variable)])
        val_ucCont <- (row_use$ucDC_pcnt[row_use$type == use_type & grepl("_C", row_use$variable)])
      }
...
setwd("~/Experiments/ICH_MRI/Ben files/MRI_ICH")


# New CS score data
new_CS <- read.xlsx("../../Command_scores_from_daily.xlsx")  #read_excel
...
Loc <- read.xlsx("ICH-FinalCompilationFinal_v5-ASRinsulaAndBFandHypothal_filled.xlsx", sheet=1)
...
summary(Vol$Hg_vol*mean(Vol$Brain_vol)/Vol$Brain_vol)


boxplot(Loc$`MLS.[mm]`)
summary(Loc$`MLS.[mm]`)
summary(Loc$`MLS.[mm]`*mean(Vol$Brain_vol)/Vol$Brain_vol)
...
summary(Vol$Ed_vol*mean(Vol$Brain_vol)/Vol$Brain_vol)
```
...
loc_analyse.raw<-subset(data.raw,select=c(MRN,4,7:98,follow,MRI_Cs,Discharge_date,DEATH_DATE,MRI_date_Loc,mri_date))
...
#### From what I understand, this part collects all the regions below as they relate to infra_tent or supra_tent areas
#### and then checks if **ANY** of those regions have a lesion there, and then that counts as having a lesion in that area.
#### It then checks to see who has lesions in only the Left, Right, or Both sides for infra_tent and supra_tent.

#### If you run this whole script up to and including this chunk, you'll see that patient 4945220 is classified as having
#### a lesion in both the L and R for BOTH infra_tent and supra_tent.

#### This causes the patient to be removed entirely in chunk 18 (called "SPLIT TABLE BY L AND R AND THEN CALC IPSILATERAL AND CONTRALATERAL")
#### because that section first splits the table between people with L and R lesions, and then assigns ipsi/conta from there. Patient 4945220
#### doesn't have a lesion on the L or R, though. They have it on "Both", which that chuck isn't checking for.
#### I have no idea how this wasn't an issue before.
...
newLocationsforMRNs <- list(
  ...
  '6625551'='R',
  '6999193'='R',
  '6591518'='L', # ADDED BEN June 15 2018 this one was supra_tent2=="L" & infra_tent2== "R"
  '4945220'='L'  # ADDED BEN June 15 2018 this one was supra_tent2=="Both" & infra_tent2== "Both"
)
...
```{r Adding new variables, echo=FALSE,warning = FALSE}
library(dplyr)
...
# possibly have to remove the _ipsi and _conto original columns for these locations,
# or else get duplicate rows in the final tabe
loc_analyse.raw$Teg_ICH_C <- NA
loc_analyse.raw$AntPons_ICH_C <- NA
loc_analyse.raw$Teg_edema_C <- NA
loc_analyse.raw$AntPons_edema_C <- NA

loc_analyse.raw$Teg_ICH_C  <- ifelse(loc_analyse.raw[,"Teg_ICH_R"]==1|
                              loc_analyse.raw[,"Teg_ICH_L"]==1,1,0)

loc_analyse.raw$AntPons_ICH_C  <- ifelse(loc_analyse.raw[,"AntPons_ICH_R"]==1|
                              loc_analyse.raw[,"AntPons_ICH_L"]==1,1,0)

loc_analyse.raw$Teg_edema_C  <- ifelse(loc_analyse.raw[,"Teg_edema_R"]==1|
                              loc_analyse.raw[,"Teg_edema_L"]==1,1,0)


loc_analyse.raw$AntPons_edema_C  <- ifelse(loc_analyse.raw[,"AntPons_edema_R"]==1|
                              loc_analyse.raw[,"AntPons_edema_L"]==1,1,0)
...
avg2<- dat.m %>%
  ...
  filter( !(variable  %in%  c("Cerebellar tonsillar herniation","MLS [mm]", "Old stroke", "Old ICH",  "Uncal herniation (to which side) ","Transtentorial herniation ","Uncal herniation (to which side)","Transtentorial herniation","sf_hern","TH_ant_ICH_L","TH_med_ICH_L","TH_lat_ICH_L","TH_post_ICH_L",
...
avg1<- dat.m %>%
  ...
  filter( !(variable  %in%  c("Cerebellar tonsillar herniation","MLS [mm]", "Old stroke", "Old ICH",  "Uncal herniation (to which side) ","Transtentorial herniation ",
                              "Uncal herniation (to which side)","Transtentorial herniation","sf_hern","TH_ant_ICH_L","TH_med_ICH_L","TH_lat_ICH_L","TH_post_ICH_L",
"TH_ant_ICH_R","TH_med_ICH_R","TH_lat_ICH_R","TH_post_ICH_R",
"TH_ant_edema_L","TH_med_edema_L","TH_lat_edema_L","TH_post_edema_L",
"TH_ant_edema_R","TH_med_edema_R","TH_lat_edema_R","TH_post_edema_R") )) %>%
...
```{r SPLIT TABLE BY L AND R AND THEN CALC IPSILATERAL AND CONTRALATERAL, echo=FALSE, warning=FALSE}
## Patients that were set as "Both" above will be excluded here??? How was this not an issue before????
...
# try removing the columns that we made a C column for, cause they can apparently make duplicate rows
## never mind, the issue is fixed now without having to do this
# test <- loc_analyse.raw3 %>%
#   select(-Teg_ICH_contro, -Teg_edema_contro,
#          -Teg_ICH_ipsi, -Teg_edema_ipsi,
#          
#          -AntPons_ICH_contro, -AntPons_edema_contro,
#          -AntPons_ICH_ipsi, -AntPons_edema_ipsi) %>%
#   distinct()

# save final data set for later statistical analysis in another script
# good one -> openxlsx::write.xlsx(loc_analyse.raw3, file="E:/Experiments/ICH_MRI/MRI_Merged_Data_158patients.xlsx", rowNames=FALSE)
#openxlsx::write.xlsx(loc_analyse.raw3, file="/Users/rohaut/Documents/Data/Columbia/ICH_MRI/MRI_Merged_Data_158patients.xlsx", rowNames=FALSE)
...
# for follow at MRI
## Ok, now all the average info is there for MRI_Cs2 and follow2, but rows are duplicated for each. Need to select distinct rows for whichever one before using this.
avg2 <- dat2.m %>%
  filter( !(variable  %in%  c("Cerebellar tonsillar herniation","MLS [mm]", "Old stroke", "Old ICH",  "Uncal herniation (to which side) ","Transtentorial herniation ",
  ...
  "TH_post_ICH_ipsi") )) %>%
  group_by(MRI_Cs2, variable) %>%
  mutate(mean_mri = mean(value, na.rm=TRUE)) %>%
  group_by(follow2, variable) %>%
  mutate(mean_dch = mean(value, na.rm=TRUE)) %>%
  select(MRI_Cs2, follow2, variable, mean_mri, mean_dch) %>%
  distinct()

avg2$MRI_Cs2 <- as.integer(avg2$MRI_Cs2); avg2$MRI_Cs2<-as.factor(avg2$MRI_Cs2)
avg2$follow2 <- as.integer(avg2$follow2); avg2$follow2<-as.factor(avg2$follow2)

# the original one:
location_table_ICH_mri <- unique(na.omit(rbind(avg2[grep(pattern = "ICH", avg2$variable),  ],avg2[avg2$variable=="IVH",]))[,c("MRI_Cs2", "variable", "mean_mri")])
# new ones:
# I guess we don't need IVH in the edema tables
location_table_edema_mri <- unique(na.omit(rbind(avg2[grep(pattern = "edema", avg2$variable),  ],avg2[avg2$variable=="IVH",]))[,c("MRI_Cs2", "variable", "mean_mri")])
location_table_ICH_dch <- unique(na.omit(rbind(avg2[grep(pattern = "ICH", avg2$variable),  ],avg2[avg2$variable=="IVH",]))[,c("follow2", "variable", "mean_dch")])
location_table_edema_dch <- unique(na.omit(rbind(avg2[grep(pattern = "edema", avg2$variable),  ],avg2[avg2$variable=="IVH",]))[,c("follow2", "variable", "mean_dch")])


# ICH ~ follow @ MRI
ggplot(data=location_table_ICH_mri, aes(x=variable, y=mean_mri, fill=MRI_Cs2) )+
  geom_bar(stat="identity", position=position_dodge()) +
  coord_flip() +
  labs(title = "11: ICH ~ Follow @ mri")

library(RColorBrewer)
ggplot( data=location_table_ICH_mri) +
          geom_tile(aes(x=variable, y=MRI_Cs2, fill=mean_mri))+
          scale_fill_gradientn(colours=rev(brewer.pal(10,"Spectral")), limits=c(0,1))+
          coord_flip()+
          ggtitle("Plot for Sketch 11_2: ICH ~ Follow @ mri")



# edema ~ follow @ MRI
ggplot(data=location_table_edema_mri, aes(x=variable, y=mean_mri, fill=MRI_Cs2) )+
  geom_bar(stat="identity", position=position_dodge()) +
  coord_flip()+
  labs(title = "12: Edema ~ Follow @ mri")


ggplot( data=location_table_edema_mri) +
          geom_tile(aes(x=variable, y=MRI_Cs2, fill=mean_mri))+
          scale_fill_gradientn(colours=rev(brewer.pal(10,"Spectral")), limits=c(0,1))+
          coord_flip()+
          ggtitle("Plot for Sketch 11_2: edema ~ Follow @ mri")



# for follow at discharge
## NOTE: avg2 table now includes this info as well
# avg1<- dat2.m %>%
#   group_by(follow2, variable) %>%
#   filter( !(variable  %in%  c("Cerebellar tonsillar herniation","MLS [mm]", "Old stroke", "Old ICH",  "Uncal herniation (to which side) ","Transtentorial herniation ",
#                               "Uncal herniation (to which side)","Transtentorial herniation","sf_hern", 
...
ggplot(data=location_table_ICH_dch, aes(x=variable, y=mean_dch, fill=follow2) )+
  geom_bar(stat="identity", position=position_dodge()) +
  coord_flip()+
  labs(title = "13: ICH ~ Follow @ discharge")

ggplot( data=location_table_ICH_dch) +
          geom_tile(aes(x=variable, y=follow2, fill=mean_dch))+
          scale_fill_gradientn(colours=rev(brewer.pal(10,"Spectral")), limits=c(0,1))+
          coord_flip()+
          ggtitle("Plot for Sketch 11_2: ICH ~ Follow @ discharge")



ggplot(data=location_table_edema_dch, aes(x=variable, y=mean_dch, fill=follow2) )+
  geom_bar(stat="identity", position=position_dodge()) +
  coord_flip()+
  labs(title = "14: Edema ~ Follow @ discharge")

ggplot( data=location_table_edema_dch) +
          geom_tile(aes(x=variable, y=follow2, fill=mean_dch))+
          scale_fill_gradientn(colours=rev(brewer.pal(10,"Spectral")), limits=c(0,1))+
          coord_flip()+
          ggtitle("Plot for Sketch 11_2: edema ~ Follow @ discharge")





### This table SHOULD match the 4 separate ones above. or JK, the others were wrong since they get averaged over ICH and edema at the same time
#### NEW table to calculate lesion percentages per ICH/edema and conscious/unconscious at MRI/Dch ####
consc_loc_table_orig <- dat2.m %>%
  filter( !(variable  %in%  c("Cerebellar tonsillar herniation","MLS [mm]", "Old stroke", "Old ICH",  "Uncal herniation (to which side) ","Transtentorial herniation ",
                              "Uncal herniation (to which side)","Transtentorial herniation","sf_hern",
                              "TH_ant_edema_contro",  "TH_ant_edema_ipsi",  "TH_lat_edema_contro",
                              "TH_lat_edema_ipsi",  "TH_med_edema_contro",  "TH_med_edema_ipsi",
                              ...
                              "TH_med_ICH_contro",    "TH_med_ICH_ipsi",    "TH_post_ICH_contro",
                              "TH_post_ICH_ipsi") )) %>%
  # create column to group by, indicating if variable is ICH or edema
  mutate(type = if_else((grepl("ICH", variable) | variable == "IVH"), true = "ICH", 
                        false = if_else(grepl("edema", variable), true = "edema", false = "NA"))) %>%  # 
  select(MRN, follow2, MRI_Cs2, type, variable, value) %>%
  distinct() %>%
  # calculate the percentages for each conscious status at MRI and Dch per ICH/edema and brain location
  group_by(type, variable) %>%
  summarise(ucMRI_pcnt = mean(value[MRI_Cs2 == 0], na.rm = T),
         cMRI_pcnt = mean(value[MRI_Cs2 == 1], na.rm = T),
         ucDC_pcnt = mean(value[follow2 == 0], na.rm = T),
         cDC_pcnt = mean(value[follow2 == 1], na.rm = T))


# with just the numbers/counts
consc_loc_table_orig_counts <- dat2.m %>%
  filter( !(variable  %in%  c("Cerebellar tonsillar herniation","MLS [mm]", "Old stroke", "Old ICH",  "Uncal herniation (to which side) ","Transtentorial herniation ",
                              "Uncal herniation (to which side)","Transtentorial herniation","sf_hern",
                              "TH_ant_edema_contro",  "TH_ant_edema_ipsi",  "TH_lat_edema_contro",
                              "TH_lat_edema_ipsi",  "TH_med_edema_contro",  "TH_med_edema_ipsi",
  ...
  # calculate the percentages for each conscious status at MRI and Dch per ICH/edema and brain location
  group_by(type, variable) %>%
  summarise(ucMRI_pcnt = sum(value[MRI_Cs2 == 0], na.rm = T),
         cMRI_pcnt = sum(value[MRI_Cs2 == 1], na.rm = T),
         ucDC_pcnt = sum(value[follow2 == 0], na.rm = T),
         cDC_pcnt = sum(value[follow2 == 1], na.rm = T))



# woof, this table creation/saving is a mess.
names_use <- c("FCx", "PCx", "TCx", "OCx", "INS", "IC_ant", "IC_post", "Caudate", "PUT", "GP", "TH", "BF", "Hypo", "MB_peduncle", "MB", "AntPons", "Teg", "Cereb", "Vermis", "IVH")

ICH_MRI_loc_pcnt <- makeLesionCountTable(consc_loc_table_orig, names_use, time = "MRI", use_type = "ICH", count_type = "percent")
ICH_Dch_loc_pcnt <- makeLesionCountTable(consc_loc_table_orig, names_use, time = "Dch", use_type = "ICH", count_type = "percent")
edema_MRI_loc_pcnt <- makeLesionCountTable(consc_loc_table_orig, names_use, time = "MRI", use_type = "edema", count_type = "percent")
edema_Dch_loc_pcnt <- makeLesionCountTable(consc_loc_table_orig, names_use, time = "Dch", use_type = "edema", count_type = "percent")

ICH_MRI_loc_pcnt[,2:5] <- lapply(lapply(ICH_MRI_loc_pcnt[,2:5], as.numeric), round)
ICH_Dch_loc_pcnt[,2:5] <- lapply(lapply(ICH_Dch_loc_pcnt[,2:5], as.numeric), round)
edema_MRI_loc_pcnt[,2:5] <- lapply(lapply(edema_MRI_loc_pcnt[,2:5], as.numeric), round)
edema_Dch_loc_pcnt[,2:5] <- lapply(lapply(edema_Dch_loc_pcnt[,2:5], as.numeric), round)

ICH_MRI_loc_count <- makeLesionCountTable(consc_loc_table_orig_counts, names_use, time = "MRI", use_type = "ICH", count_type = "count")
ICH_Dch_loc_count <- makeLesionCountTable(consc_loc_table_orig_counts, names_use, time = "Dch", use_type = "ICH", count_type = "count")
edema_MRI_loc_count <- makeLesionCountTable(consc_loc_table_orig_counts, names_use, time = "MRI", use_type = "edema", count_type = "count")
edema_Dch_loc_count <- makeLesionCountTable(consc_loc_table_orig_counts, names_use, time = "Dch", use_type = "edema", count_type = "count")

# convert all to matrices with rownames as that first column
ICH_MRI_loc_pcnt.mx <- as.matrix(ICH_MRI_loc_pcnt[,2:5])
rownames(ICH_MRI_loc_pcnt.mx) <- ICH_MRI_loc_pcnt$name
ICH_Dch_loc_pcnt.mx <- as.matrix(ICH_Dch_loc_pcnt[,2:5])
rownames(ICH_Dch_loc_pcnt.mx) <- ICH_Dch_loc_pcnt$name
edema_MRI_loc_pcnt.mx <- as.matrix(edema_MRI_loc_pcnt[,2:5])
rownames(edema_MRI_loc_pcnt.mx) <- edema_MRI_loc_pcnt$name
edema_Dch_loc_pcnt.mx <- as.matrix(edema_Dch_loc_pcnt[,2:5])
rownames(edema_Dch_loc_pcnt.mx) <- edema_Dch_loc_pcnt$name



#a <- formatC(as.matrix(ICH_MRI_loc_pcnt[,2:5]))
ICH_MRI_loc_out <- matrix(
  paste0(ICH_MRI_loc_pcnt.mx, "% (N=", as.matrix(ICH_MRI_loc_count[,2:5]), ")"),
  nrow = nrow(ICH_MRI_loc_pcnt),
              dimnames=dimnames(ICH_MRI_loc_pcnt.mx)
)

ICH_Dch_loc_out <- matrix(
  paste0(ICH_Dch_loc_pcnt.mx, "% (N=", as.matrix(ICH_Dch_loc_count[,2:5]), ")"),
  nrow = nrow(ICH_Dch_loc_pcnt.mx),
              dimnames=dimnames(ICH_Dch_loc_pcnt.mx)
)

edema_MRI_loc_out <- matrix(
  paste0(edema_MRI_loc_pcnt.mx, "% (N=", as.matrix(edema_MRI_loc_count[,2:5]), ")"),
  nrow = nrow(ICH_Dch_loc_pcnt.mx),
              dimnames=dimnames(edema_MRI_loc_pcnt.mx)
)

edema_Dch_loc_out <- matrix(
  paste0(edema_Dch_loc_pcnt.mx, "% (N=", as.matrix(edema_Dch_loc_count[,2:5]), ")"),
  nrow = nrow(edema_Dch_loc_pcnt.mx),
              dimnames=dimnames(edema_Dch_loc_pcnt.mx)
)


# then paste everything from these tables into the word table
write.csv(ICH_MRI_loc_out, "../ICH_MRI_loc_out.csv", row.names = T)
write.csv(ICH_Dch_loc_out, "../ICH_Dch_loc_out.csv", row.names = T)
write.csv(edema_MRI_loc_out, "../edema_MRI_loc_out.csv", row.names = T)
write.csv(edema_Dch_loc_out, "../edema_Dch_loc_out.csv", row.names = T)



### This is to check the denominators for the columns
denom_check <- dat2.m %>%
  filter( !(variable  %in%  c("Cerebellar tonsillar herniation","MLS [mm]", "Old stroke", "Old ICH",  "Uncal herniation (to which side) ","Transtentorial herniation ",
                              "Uncal herniation (to which side)","Transtentorial herniation","sf_hern",
                              "TH_ant_edema_contro",  "TH_ant_edema_ipsi",  "TH_lat_edema_contro",
...
n_pats_check <- denom_check %>%
  group_by(type) %>%
  summarise(n_pat_cMRI = length(unique(MRN[MRI_Cs2 == 1])),
         n_pat_ucMRI = length(unique(MRN[MRI_Cs2 == 0])),
         n_pat_cDch = length(unique(MRN[follow2 == 1])),
         n_pat_ucDch = length(unique(MRN[follow2 == 0])))

# save file with IVH info
IVH_info <- denom_check %>%
  filter(variable == "IVH") %>%
  distinct() %>%
  select(MRN, value) %>%
  rename(Has_IVH = value)

write.csv(IVH_info, "../Has_IVH_info.csv", row.names = F)

```



```{r new conscious state between MRI and Dch grouping of patients to make new figures}



# can make this table all in one go:
consc_loc_table <- dat2.m %>%
  filter( !(variable  %in%  c("Cerebellar tonsillar herniation","MLS [mm]", "Old stroke", "Old ICH",  "Uncal herniation (to which side) ","Transtentorial herniation ",
                              "Uncal herniation (to which side)","Transtentorial herniation","sf_hern",
                              "TH_ant_edema_contro",  "TH_ant_edema_ipsi",  "TH_lat_edema_contro",
                              "TH_lat_edema_ipsi",  "TH_med_edema_contro",  "TH_med_edema_ipsi",
  ...
  distinct() %>%
  group_by(variable) %>%
  summarise(ucMRIucDC_pcnt = mean(value[MRI_Cs2 == 0 & follow2 == 0], na.rm = T),
         ucMRIcDC_pcnt = mean(value[MRI_Cs2 == 0 & follow2 == 1], na.rm = T),
         cMRIcDC_pcnt = mean(value[MRI_Cs2 == 1 & follow2 == 1], na.rm = T),
         cMRIucDC_pcnt = mean(value[MRI_Cs2 == 1 & follow2 == 0], na.rm = T))


# now melt this so I can plot it
consc_loc_table.m <- reshape2::melt(consc_loc_table, id.vars = "variable", variable.name = "conscious_status", value.name = "percent")



# plots
# ggplot( data=location_table_ICH_dch) +
#           geom_tile(aes(x=follow2, y=variable, fill=mean_dch))+
#           scale_fill_gradientn(colours=rev(brewer.pal(10,"Spectral")), limits=c(0,1))+
#           #coord_flip()+
#           ggtitle("Plot for Sketch 11_2: ICH ~ Follow @ discharge")
# 
# ggplot( data=ucMRIucDC) +
#           geom_tile(aes(x="1", y=variable, fill=percent))+
#           scale_fill_gradientn(colours=rev(brewer.pal(10,"Spectral")), limits=c(0,1))+
#           #coord_flip()+
#           ggtitle("Plot for Sketch 11_2: ICH ~ unconsc MRI; unconsc Dch")

## x-axis name key:
## ucMRIucDC == unconscious at MRI and unconscious at discharge
## ucMRIcDC == unconscious at MRI and conscious at discharge
## cMRIcDC == conscious at MRI and conscious at discharge


# plot
ggplot( data = consc_loc_table.m) +
          geom_tile(aes(x = conscious_status, y = variable, fill = percent))+
          scale_fill_gradientn(colours=rev(brewer.pal(10,"Spectral")), limits=c(0,1))+
          #coord_flip()+
          ggtitle("Plot for Sketch 11_X: ICH ~ conscious trajectory")
...
avg3<- dat2.m %>%
  ...
  "TH_post_edema_contro", "TH_post_edema_ipsi", "TH_ant_ICH_contro",
  "TH_ant_ICH_ipsi",    "TH_lat_ICH_contro",    "TH_lat_ICH_ipsi",
  "TH_med_ICH_contro",    "TH_med_ICH_ipsi",    "TH_post_ICH_contro", "Meso_ICH_2_ipsi",
  "TH_post_ICH_ipsi") )) %>%
  #summarize(mean=mean(value,na.rm=TRUE), sd=sd(value,na.rm=TRUE)) no need sd (%)
  summarize(mean=mean(value,na.rm=TRUE))


ggplot(data=na.omit(rbind(avg3[grep(pattern = "ICH", avg3$variable),  ],avg3[avg3$variable=="IVH",])), aes(x=variable, y=mean) )+
  geom_bar(stat="identity", position=position_dodge()) +
  coord_flip()+
  labs(title = "22: ICH ~ all patients")


# HERE


ggplot( data=na.omit(rbind(avg3[grep(pattern = "ICH", avg3$variable),  ],avg3[avg3$variable=="IVH",]))) +
          geom_tile(aes(x=variable, y=0, fill=mean))+
          scale_fill_gradientn(colours=rev(brewer.pal(10,"Spectral")), limits=c(0,1))+
          coord_flip()+
          ggtitle("Plot for Sketch 22_2: ICH ~ all patients")
