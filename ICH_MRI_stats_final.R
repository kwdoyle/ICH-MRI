...
# not used
RunModelNew <- function(data, indep_var_ix, dep_var_vals, n_runs, lambda_grid = NULL, p = 0.80, alpha = 0.95, 
                        nfolds = 10, lambda_use = "lambda.min") {
  df <- data.matrix(data[, indep_var_ix], rownames.force = T)
  # add dependent variable to beginning of table
  df <- cbind(as.factor(dep_var_vals), df)
...
# same function as above, but performs Leave-One-Out cross validation 
# (not used here, actually.. this was for testing earlier; this model gives very similar results to the train/test 
# on 80% model as performed in the above function)
# (not used)
RunLOOModel <- function(data, indep_var_ix, dep_var_vals, lambda_grid = NULL, 
                        p = 0.80, alpha = 0.95, nfolds = 10, lambda_use = "lambda.min") {
...
# (not used)
bestAlphaLambda <- function(data, indep_var_ix, dep_var_vals, lambda.grid, alpha.grid, 
                            method = "repeatedcv", p = 0.8, number = 10, repeats = 5) {
...
names(loc_analyse.raw3)[which(names(loc_analyse.raw3) == 'MLS_[mm]')] <- "MLS"
...
loc_analyse.raw3 <- loc_analyse.raw3[, -which(names(loc_analyse.raw3) == "Meso_ICH_2_ipsi" | 
                                                names(loc_analyse.raw3) == "Meso_ICH_2_contro")]
...
new_CS <- read.xlsx("../Command_scores_and_days.xlsx")
...
#### Load sedation data ####
# sedation data is now in an rds "database"
seddatM8GS <- readRDS("../Sedation/sedation_M8GS.rds")
## Who do we have?
...
#      !!! But need to remove the row that has an NA or 0 for the med_amount !!! 
#      (only remove med_amount == 0 or is NA for the DUPLICATES)
...
#     this will take care of those that are actually missing and don't have the value filled in on the other flowsheet
#     or in a second row
...
### step 1 might not be necessary anymore, depending on whether or not the duplicates aren't actually duplicates
# # 1)
...
# # so this has actual entire row duplicates AND not-fully-filled row duplicates 
# # (because of the multiple flowsheet entries)
...
for (i in 1:length(med_na)) {
  ...
  if (!is.na(rw$Concentration)) {
    if (rw$Concentration != 1) {
      stop(print(paste("something's wrong with row", med_na[i])))
    }
  }
  # for propofol, check if dispensed amt units are in mL and then multiply 
  # the default concentration of 10 mg/mL to get med amount
...
sedMRI <- ichmri %>%
  ...
  # Therefore, it's not included in this because it's only taking the Medication_Amt column, 
  # which is made using those concentrations
  # Ok, apparently SOME injections still have this column calculated somehow without a concentration
  # this is because SOME of the injections have their dispensed amount units in micrograms instead of milliliters.
  
  filter( (Medication_Name == "Propofol +R DRIP" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Prop_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date)) |
          (Medication_Name == "Propofol Inj" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Prop_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date)) |
          (Medication_Name == "Propofol Inj +R+" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Prop_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date)) |
            
          (Medication_Name == "Midazolam HCL Inj +R+" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Midaz_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date)) |
          (Medication_Name == "Midazolam HCl +R+ DRIP" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Midaz_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date)) |
            
          (Medication_Name == "Dexmedetomidine Drip 400 microgram/100ml" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Dexmed_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date)) |
          (Medication_Name == "Dexmedetomidine Drip 200 microgram/50ml" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Dexmed_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date)) |
          (Medication_Name == "Dexmedetomidine Drip" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Dexmed_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date)) |
            
          (Medication_Name == "fentaNYL DRIP" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Fent_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date)) |
          (Medication_Name == "Fentanyl DRIP" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Fent_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date)) |
          (Medication_Name == "fentaNYL DRIP." & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Fent_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date)) |
          (Medication_Name == "fentaNYL DRIP 5000 microgram/250ml" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Fent_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date)) |
          (Medication_Name == "Fentanyl Citrate Inj +R+" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Fent_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date)) |
          (Medication_Name == "fentaNYL Citrate Inj +R+" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Fent_2halflife) & 
          ...
          (Medication_Name == "Ketamine DRIP" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Keta_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date)) |
          (Medication_Name == "Ketamine Inj" & 
             (Administered_Dtm >= MRI_eval_note_date - (60 * 60 * Keta_2halflife) & 
                Administered_Dtm <= MRI_eval_note_date))  ) %>%
  ...
  mutate(Assessment_Time_Drug = case_when(Administered_Dtm %within% interval(MRI_eval_note_date - (60 * 60),
                                                                             MRI_eval_note_date) ~ Medication_Amt_mg,
                                          TRUE ~ 0)) %>%
  # so now make a column of just the dates or group by date(MRI_eval_note_date)?? 
  # and then take sum of Medication_Amt_mg and the max of Assessment_Time_Drug
  group_by(MRN, MRI_eval_note_date, Medication_Name) %>%
  summarize(Drug_Sum_mg = sum(Medication_Amt_mg, na.rm = T), 
            Assessment_Time_Drug = max(Assessment_Time_Drug, na.rm = T))
...
########### Right now this is ONLY looking at DRIPS and INJECTIONS THAT WERE IN MICROGRAMS 
# instead of mLs for dispensed amount.
sedMRI <- sedMRI %>%
...
sedb4MRI <- dcast(sedMRI, MRN + MRI_eval_note_date ~ New_Drug_Name, 
                  value.var = "Drug_Sum_mg", fill = 0, fun.aggregate = sum)
sedatMRI <- dcast(sedMRI, MRN + MRI_eval_note_date ~ New_Drug_Name, 
                  value.var = "Assessment_Time_Drug", fill = 0, fun.aggregate = sum)
# ...or maybe not
...
drugfit_b4 <- glm(MRI_Cs2 ~ Dexmedetomidine * Fentanyl * Midazolam * Propofol, 
                  data = sedConscb4MRI, family = "binomial")

summary(drugfit_b4)
# apparently they don't.
# rather, they don't play an affect in our dataset on predicting consciousness.
drugfit_at <- glm(MRI_Cs2 ~ Dexmedetomidine * Fentanyl * Midazolam * Propofol, 
                  data = sedConscatMRI, family = "binomial")

summary(drugfit_at)
# fentanyl is marginally significant when looking at just the amounts given at most an hour before the assessment.
# the fact that the coefficients of the others are positive probablly means 
# that many conscious patients had a value of 0 for the drug.
...
#                                 Dexmedetomidine:Propofol + Fentanyl:Propofol + Midazolam:Propofol, 
#                               data = sedConscatMRI, family = "binomial")
#
# summary(drugfit_at_justdoubles)
# see if this matches
drugfit_at2 <- glm(MRI_Cs2 ~ Dexmedetomidine_at_MRI * Fentanyl_at_MRI * Midazolam_at_MRI * Propofol_at_MRI, 
                   data = loc_analyse.raw3, family = "binomial")
summary(drugfit_at2)

drugfit_b4_Dch <- glm(follow2 ~ Dexmedetomidine * Fentanyl * Midazolam * Propofol, 
                      data = sedConscb4MRI_Dch, family = "binomial")
summary(drugfit_b4_Dch)

# apparently they don't.
# rather, they don't play an affect in our dataset on predicting consciousness.
drugfit_at_Dch <- glm(follow2 ~ Dexmedetomidine * Fentanyl * Midazolam * Propofol, 
                      data = sedConscatMRI_Dch, family = "binomial")
...
loc_analyse.raw3[,c("Dexmedetomidine_before_MRI_MeanNorm",
                    "Fentanyl_before_MRI_MeanNorm",
                    "Midazolam_before_MRI_MeanNorm",
                    "Propofol_before_MRI_MeanNorm",
                    "Dexmedetomidine_at_MRI_MeanNorm",
                    "Fentanyl_at_MRI_MeanNorm",
                    "Midazolam_at_MRI_MeanNorm",
                    "Propofol_at_MRI_MeanNorm")] <- sapply(loc_analyse.raw3[,c("Dexmedetomidine_before_MRI",
                                                                               "Fentanyl_before_MRI",
                                                                               "Midazolam_before_MRI",
                                                                               "Propofol_before_MRI",
                                                                               "Dexmedetomidine_at_MRI",
                                                                               "Fentanyl_at_MRI",
                                                                               "Midazolam_at_MRI",
                                                                               "Propofol_at_MRI")],
                                                                                meanNormalization)
...
labs <- read.xlsx("../All_Labs_4d_Window.xlsx")
...
loc_analyse.raw3[,c("BUN_MeanNorm", "Creat_MeanNorm", "Gluc_MeanNorm")] <- lapply(
  loc_analyse.raw3[,c("BUN", "Creat", "Gluc")], meanNormalization
  )


#### Load outcome data ####
outcomes <- read.xlsx("../ICH_MRI_Patient_Outcomes.xlsx")
...
FUNC_GCS_ICH <- read.xlsx("../FUNC_GCS_ICH_Score_158_MRI_patients.xlsx")
...
#### get all parameters ####
# there was probably a better way to pick the relevant column names to use...
...
med_b4_ix <- which(names(loc_analyse.raw3) == "Dexmedetomidine_before_MRI_MeanNorm" | 
                     names(loc_analyse.raw3) == "Fentanyl_before_MRI_MeanNorm" | 
                     names(loc_analyse.raw3) == "Midazolam_before_MRI_MeanNorm" | 
                     names(loc_analyse.raw3) == "Propofol_before_MRI_MeanNorm")

## Use this one for sed meds
med_at_ix <- which(names(loc_analyse.raw3) == "Dexmedetomidine_at_MRI_MeanNorm" | 
                     names(loc_analyse.raw3) == "Fentanyl_at_MRI_MeanNorm" |
                     names(loc_analyse.raw3) == "Midazolam_at_MRI_MeanNorm" | 
                     names(loc_analyse.raw3) == "Propofol_at_MRI_MeanNorm")

## Lab values
lab_ix <- which(names(loc_analyse.raw3) %in% c("BUN_MeanNorm","Gluc_MeanNorm"))

## all_ix is ONLY including sedation at time of MRI
## JUST KIDDING it has both now
all_meds <- c(med_b4_ix, med_at_ix)

all_ix <- c(ich_ix, ed_ix, other_ix, all_meds, lab_ix)
...
all_ix2 <- c(ich_ix, ed_ix, other_ix2, med_at_ix, lab_ix)
...
ipsi_TH_names <- grep("TH", grep("ipsi", names(loc_analyse.raw3), ignore.case = T, value = T), 
...
ipsi_PUT_names <- grep("PUT", grep("ipsi", names(loc_analyse.raw3), ignore.case = T, value = T), 
                       ignore.case = T, value = T)
...
# There's too few observations to do the cross-validation (only 62 patients now)
...
length(which(Meso_Vol_Dch2$medProb$class_vals == 1 & 
               Meso_Vol_Dch2$medProb$opposite_vals == 0 & 
               Meso_Vol_Dch2$medProb$actual_vals == 1))

length(which(Meso_Vol_Dch2$medProb$class_vals == 0 & 
               Meso_Vol_Dch2$medProb$opposite_vals == 0 & 
               Meso_Vol_Dch2$medProb$actual_vals == 1))
# and for the other cases
length(which(Meso_Vol_Dch2$medProb$class_vals == 1 & 
               Meso_Vol_Dch2$medProb$opposite_vals == 1 & 
               Meso_Vol_Dch2$medProb$actual_vals == 1))
...
# ...I don't actually ever use this list.
...
true_neg_ix <- All_params_MRI$medProb$patient[which(All_params_MRI$medProb$class_vals == 0 & 
                                                      All_params_MRI$medProb$actual_vals ==0)]
false_neg_ix <- All_params_MRI$medProb$patient[which(All_params_MRI$medProb$class_vals == 0 & 
                                                       All_params_MRI$medProb$actual_vals == 1)]
true_pos_ix <- All_params_MRI$medProb$patient[which(All_params_MRI$medProb$class_vals == 1 & 
                                                      All_params_MRI$medProb$actual_vals == 1)]
false_pos_ix <- All_params_MRI$medProb$patient[which(All_params_MRI$medProb$class_vals == 1 & 
                                                       All_params_MRI$medProb$actual_vals == 0)]
...
# Why did we hard-code these numbers in?!
...
# MRI
ggplot(data = filter(histdat.newnms, grepl("_MRI", variable)), aes(x = reorder(variable, value), 
...
## Dch
ggplot(data = filter(histdat.newnms, grepl("_Dch", variable)), aes(x = reorder(variable, value), 
                                                                   y = value, fill = variable)) +
  geom_violin(alpha = 0.7, position = "identity", draw_quantiles = c(0.25, 0.5, 0.75)) +
  geom_hline(yintercept = 0.5) +
  scale_y_continuous(breaks = scales::pretty_breaks(n = 9)) +
...
MesoVolGOSE_reshape <- reshape2::dcast(as.data.frame(MesoVolGOSEcounts), GOSE_All ~ class_grp, 
...
newer_param_order <- c("FCx_ICH_ipsi","FCx_edema_ipsi","FCx_ICH_contra","FCx_edema_contra",
                     ...
                     # added these two after SciRep R1 1/28/19
...
ggplot(filter(melt(All_params_Dch_newnames), variable != "(Intercept)"), aes(x = reorder(variable, -value, FUN = median), y = value)) + 
  ...
  ylab("Coefficient") +
  xlab("Parameter / Brain Region") +
  scale_x_discrete(limits = rev(newer_param_order_dch)) +
  coord_flip()
...
# write.xlsx(BSD_Vol_Dch_Weight_Table_formatted, 
#            "E:/Experiments/ICH_MRI/Weight Tables/BSD_Vol_Dch_weights.xlsx", rowNames = T)
# write.xlsx(BSD_Vol_MRI_Weight_Table_formatted, 
#            "E:/Experiments/ICH_MRI/Weight Tables/BSD_Vol_MRI_weights.xlsx", rowNames = T)
# write.xlsx(BSD_Dch_Weight_Table_formatted, 
#            "E:/Experiments/ICH_MRI/Weight Tables/BSD_Dch_weights.xlsx", rowNames = T)
# write.xlsx(BSD_MRI_Weight_Table_formatted, 
#            "E:/Experiments/ICH_MRI/Weight Tables/BSD_MRI_weights.xlsx", rowNames = T)
# 
# write.xlsx(BSD_Vol_IVH_Dch_weight_Table_formatted, 
#            "E:/Experiments/ICH_MRI/Weight Tables/BSD_Vol_IVH_Dch_weights.xlsx", rowNames = T)
# write.xlsx(BSD_Vol_IVH_MRI_weight_Table_formatted, 
#            "E:/Experiments/ICH_MRI/Weight Tables/BSD_Vol_IVH_MRI_weights.xlsx", rowNames = T)
...
barplot_dat <- melt(loc_analyse.ipsi_TH[c(which(names(loc_analyse.raw3) %in% c("MRN", "follow2")),
                                          all_ix3[-Extra_ix_to_remove])], id.vars = c("MRN", "follow2"))  #, "follow", "MRI_Cs", "Discharge_date", "DEATH_DATE", "MRI_date_Loc", "mri_date", "infra_tent", 
