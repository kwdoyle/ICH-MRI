...
# only include regions that have 5 or more patients with legions there
avg3_2 <- filter(avg3, group_tot >= 5)
...
rukiddingme <- data.matrix(na.omit(cbind(coltotalsTable, coltotalsTable)))

# this doesn't seem to work either. the colors are wrong.
gplots::heatmap.2(rukiddingme, trace="n", Colv=NA, labCol="", 
                  labRow=rownames(coltotalsTable), cexRow=0.75)

heatmap(aaa)

why <- data.matrix(iris)[,1:4]
ugh <- cbind(why[,1], why[,1])

heatmap(ugh, Colv=F, Rowv=F)

guh <- cbind(rownames(coltotalsTable), coltotalsTable)

#image(data.matrix(guh), col=heat.colors(1))
image(data.matrix(coltotalsTable), col=grey.colors(5000), xlab=rownames(coltotalsTable))
