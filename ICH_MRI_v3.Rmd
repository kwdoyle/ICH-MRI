...
Loc <- read_excel("ICH-FinalCompilationFinal.xlsx", sheet=1)
...
# set to NA only if there ARE values equal to 0 (otherwise get error)
if (length(Vol[!is.na(Vol$Brain_vol) & Vol$Brain_vol == 0,]$Brain_vol) != 0) {
...
# Control of  brain volumes  # to be fixed
boxplot(Vol$Hg_vol)
if (length(Vol[!is.na(Vol$Hg_vol) & Vol$Hg_vol == 0,]$Hg_vol) != 0) {
...
### I think the duplicates were removed in the new updated file?
...
#aaa <- merge(Loc2, Outcome, by.x= c("MRN"), all.x=TRUE, all.y=FALSE) 
data.raw <-merge(Loc, Outcome, by.x= c("MRN"), by.y=c("MRN"), all.x=TRUE, all.y=FALSE)   # 
...
```{r Bayes stat test (and other analyses), include = FALSE}
library(BayesFactor)
...
# tables should look like: (and need a seperate table for each model)
...
Table: Example table for Latent Variable Analysis Results
